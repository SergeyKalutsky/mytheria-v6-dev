...
public class NeVasilekFarmilka extends BaseModule {
   ...
   private void setState(NeVasilekFarmilka.State newState) {
      NeVasilekFarmilka.State old = this.state;
      this.state = newState;
      if (old != newState) {
   ...
   private void setCraftStage(NeVasilekFarmilka.CraftStage newStage) {
      NeVasilekFarmilka.CraftStage old = this.craftStage;
   ...
   private static final NeVasilekFarmilka.SwordEnchantRequirement[] SWORD_REQUIREMENTS = new NeVasilekFarmilka.SwordEnchantRequirement[]{
      new NeVasilekFarmilka.SwordEnchantRequirement("sharpness", new String[]{"Острота", "Sharpness"}, 7),
      new NeVasilekFarmilka.SwordEnchantRequirement("smite", new String[]{"Кара", "Smite"}, 7),
      new NeVasilekFarmilka.SwordEnchantRequirement("bane", new String[]{"Бич", "Bane of Arthropods"}, 7),
      new NeVasilekFarmilka.SwordEnchantRequirement("sweeping", new String[]{"Разящ", "Sweeping Edge"}, 3),
      new NeVasilekFarmilka.SwordEnchantRequirement("fire", new String[]{"Огня", "Fire Aspect"}, 2),
      new NeVasilekFarmilka.SwordEnchantRequirement("looting", new String[]{"Добыч", "Looting"}, 5),
      new NeVasilekFarmilka.SwordEnchantRequirement("unbreaking", new String[]{"Прочн", "Unbreaking"}, 5),
      new NeVasilekFarmilka.SwordEnchantRequirement("expert", new String[]{"Опытн"}, 3),
      new NeVasilekFarmilka.SwordEnchantRequirement("vampirism", new String[]{"Вампиризм", "Vampirism"}, 2),
      new NeVasilekFarmilka.SwordEnchantRequirement("oxidation", new String[]{"Окисление", "Oxidation"}, 2),
   ...
   private final List<NeVasilekFarmilka.PriceRequest> priceRequests;
   private int priceIndex;
   private final Map<String, NeVasilekFarmilka.PriceCache> priceCache;
   private final Map<String, Long> unitPrices;
   private final List<NeVasilekFarmilka.BuyRequest> buyRequests;
   ...
   private NeVasilekFarmilka.State pendingOpenState;
            ...
            if (!this.isAnarchyBossBarActive) {
               if (this.afkRetryActive) {
                  ...
                  if (this.isRelistState()) {
                     ...
                     this.handleBuyClose();
                     if (this.state == NeVasilekFarmilka.State.IDLE && this.relistPending && this.shouldStartRelist() && mc.field_1755 == null) {
                        this.setState(NeVasilekFarmilka.State.RELIST_OPEN_AH);
   ...
   private void startAccountQueue() {
      if (!this.isAccountQueueEnabled()) {
         ...
         if (this.isMainAccountMode() && (this.queueServer == null || !this.queueServer.isRunning())) {
            this.queueServer = new NeVasilekFarmilka.QueueCoordinatorServer(var1);
            this.queueServer.start();
            MessageUtility.info(class_2561.method_43470("[NeVasilekFarmilka] Локальный сервер очереди запущен на порту " + var1));
         }
         ...
         if (this.queueClient == null || !this.queueClient.isRunning() || !this.queueClient.isFor(this.queueAccountId, var1, this.isMainAccountMode())) {
            ...
            this.queueClient = new NeVasilekFarmilka.QueueCoordinatorClient("127.0.0.1", var1, this.queueAccountId, this.isMainAccountMode());
   ...
   private void handleAccountQueueTick() {
      if (this.isAccountQueueEnabled()) {
         ...
         if (this.queueClient != null && this.queueClient.consumeJustConnected()) {
            MessageUtility.info(class_2561.method_43470("[NeVasilekFarmilka] Аккаунт подключен к локальному серверу очереди."));
         }

         if (this.isMainAccountMode() && this.queueServer != null) {
            String var1 = this.queueServer.consumeLastConnectedAccountId();
            if (var1 != null && !var1.isBlank() && !var1.equals(this.queueAccountId)) {
               MessageUtility.info(class_2561.method_43470("[NeVasilekFarmilka] К серверу очереди подключен аккаунт: " + var1));
            }
         }
   ...
   private boolean isBuyState() {
      return this.state == NeVasilekFarmilka.State.BUY_SEND
         || this.state == NeVasilekFarmilka.State.BUY_WAIT_SCREEN
         || this.state == NeVasilekFarmilka.State.BUY_CLICK
         || this.state == NeVasilekFarmilka.State.BUY_WAIT_RESULT;
   }
   ...
   private void handleIdle() {
      if (mc.field_1755 == null) {
         if (this.shouldPriceCheck() && this.needsPriceUpdate()) {
            ...
            this.setState(NeVasilekFarmilka.State.USE_XP);
            this.actionTimer.reset();
         } else {
               ...
               if (this.shouldUpgradeFinalSword()) {
                  if (this.countItem(class_1802.field_22020) <= 0) {
                     ...
                     if (!this.buyRequests.isEmpty()) {
                        this.setState(NeVasilekFarmilka.State.BUY_SEND);
                        this.actionTimer.reset();
                        return;
                     }
                  }

                  this.setState(NeVasilekFarmilka.State.SMITH_OPEN);
                  this.actionTimer.reset();
                  return;
               }
            ...
            if ((var1 = this.findPickaxeSlotWithBad()) != -1) {
               ...
               if (this.isSwordMode() && this.hasKnockbackEnchant(var3)) {
                  ...
                  this.dropSlot = var1;
                  this.setState(NeVasilekFarmilka.State.DROP_BAD);
                  this.actionTimer.reset();
               }
            } else {
               ...
               if (var4 != -1) {
                  ...
                  this.setState(NeVasilekFarmilka.State.CRAFT_OPEN);
                  this.actionTimer.reset();
               } else if (this.hasUnenchantedPickaxe()) {
                  this.setState(NeVasilekFarmilka.State.ENCHANT_OPEN);
                  this.actionTimer.reset();
               }
            }
         }
   ...
   private void handlePriceSend() {
      if (this.priceIndex >= this.priceRequests.size()) {
         this.setState(NeVasilekFarmilka.State.DECIDE);
      } else {
         NeVasilekFarmilka.PriceRequest var1 = this.priceRequests.get(this.priceIndex);
         NeVasilekFarmilka.PriceCache var2 = this.priceCache.get(var1.key);
         if (var2 != null && var2.isValid()) {
            ...
            this.searchTimer.reset();
            this.setState(NeVasilekFarmilka.State.PRICE_WAIT);
         }
      }
   }
         ...
         if (mc.field_1755 instanceof class_465 var2) {
            ...
            if (!this.isSearchScreen()) {
               ...
               if (System.currentTimeMillis() - this.searchScreenOpenedAtMs >= 300L && this.searchTimer.finished(this.delayMs())) {
                  NeVasilekFarmilka.PriceRequest var4 = this.priceRequests.get(this.priceIndex);
                  ...
                  if (var5 > 0L) {
                     this.priceCache
                        .put(
                           var4.key,
                           new NeVasilekFarmilka.PriceCache(var5, System.currentTimeMillis() + this.cacheMs(), var4.output ? this.outputPricePerQuality : -1.0)
                        );
                  }

                  if (this.closeHandledScreenSafely()) {
                     this.priceIndex++;
                     this.setState(NeVasilekFarmilka.State.PRICE_SEND);
                     this.actionTimer.reset();
                  }
               }
            }
   ...
   private void handleBuySend() {
      if (this.buyIndex >= this.buyRequests.size()) {
         ...
         NeVasilekFarmilka.BuyRequest var1 = this.buyRequests.get(this.buyIndex);
         if (this.isBuySatisfied(var1)) {
            ...
            if (this.tryConfirmPurchase()) {
               ...
               this.setState(NeVasilekFarmilka.State.BUY_WAIT_RESULT);
            } else {
            ...
            this.searchTimer.reset();
            this.setState(NeVasilekFarmilka.State.BUY_WAIT_SCREEN);
         }
      }
   }
      ...
      if (mc.field_1755 instanceof class_465 var2) {
         ...
         if (!var3.method_34255().method_7960()) {
            ...
            if (this.searchFailed) {
               ...
               this.setState(NeVasilekFarmilka.State.BUY_CLICK);
            }
         }
      } else if (this.searchTimer.finished(this.timeoutMs())) {
         this.buyScreenOpenedAtMs = 0L;
      ...
      if (!this.isPickaxeMode() && !this.isSwordMode()) {
         ...
         if (mc.field_1755 == null && this.actionTimer.finished(this.delayMs())) {
            ...
            if (var1 == null) {
               ...
               this.beginBlockOpen(var1, NeVasilekFarmilka.State.CRAFT_WAIT);
               this.craftTimer.reset();
               this.setCraftStage(NeVasilekFarmilka.CraftStage.NONE);
               this.craftFailTicks = 0;
               this.setState(NeVasilekFarmilka.State.CRAFT_WAIT);
            }
         }
      }
   }
   ...
   private void handleCraftWait() {
      if (mc.field_1755 instanceof class_465 var2) {
         if (var2.method_17577() instanceof class_1714) {
            ...
            this.craftTimer.reset();
            this.setState(NeVasilekFarmilka.State.CRAFTING);
   ...
   private void handleCrafting() {
      if (!(mc.field_1755 instanceof class_465 var2 && var2.method_17577() instanceof class_1714 var3)) {
         ...
         moscow.mytheria.logger.MytheriaLogger.event("craft_dispatch")
            ...
            .emit();
         if (this.craftStage == NeVasilekFarmilka.CraftStage.NONE) {
            this.closeHandledScreenSafely();
            this.setState(NeVasilekFarmilka.State.IDLE);
         } else {
            boolean var5 = false;
   ...
   private void handleEnchantOpen() {
      if (mc.field_1755 == null && this.actionTimer.finished(this.delayMs())) {
         ...
         if (var1 == null) {
            ...
            this.beginBlockOpen(var1, NeVasilekFarmilka.State.ENCHANT_WAIT);
            this.enchantTimer.reset();
            this.setState(NeVasilekFarmilka.State.ENCHANT_WAIT);
         }
      }
   }
            ...
            if (var5 == -1) {
               ...
               if (var6 != -1 && var7 != -1) {
                  ...
                  if (!var9.method_7960() && var9.method_7909() != class_1802.field_8759) {
                     ...
                     this.enchantTimer.reset();
                     this.setState(NeVasilekFarmilka.State.ENCHANT_CLICK);
                  }
               } else {
                  this.closeHandledScreenSafely();
                  this.setState(NeVasilekFarmilka.State.IDLE);
   ...
   private void handleEnchantClick() {
      if (!(mc.field_1755 instanceof class_465 var2 && var2.method_17577() instanceof class_1718 var3)) {
         ...
         if (var7 > 0 && mc.field_1724.field_7520 < var7) {
            MessageUtility.info(class_2561.method_43470("[NeVasilekFarmilka] Not enough XP for enchant (need " + var7 + "), closing to use bottles."));
            if (this.closeHandledScreenSafely()) {
               this.setState(NeVasilekFarmilka.State.IDLE);
               this.enchantTimer.reset();
            }
            ...
            this.enchantTimer.reset();
            this.setState(NeVasilekFarmilka.State.ENCHANT_WAIT_RESULT);
         }
      }
   }
      ...
      if (mc.field_1755 instanceof class_465 var2 && var2.method_17577() instanceof class_1718 var3) {
         if (!var3.method_34255().method_7960()) {
            ...
            if (this.findUnenchantedPickaxeSlot() != -1
               ...
               this.setState(NeVasilekFarmilka.State.EVALUATE);
               this.enchantTimer.reset();
            }
         }
      } else {
   ...
   private void handleGrindOpen() {
      ...
      if (mc.field_1755 == null && this.actionTimer.finished(this.delayMs())) {
         ...
         if (var3 == null) {
            ...
            this.beginBlockOpen(var3, NeVasilekFarmilka.State.GRIND_WAIT);
            this.craftTimer.reset();
            this.setState(NeVasilekFarmilka.State.GRIND_WAIT);
         }
      }
   }
      ...
      if (!(mc.field_1755 instanceof class_465 var2 && var2.method_17577() instanceof class_3803 var3)) {
         ...
         if (var8 == -1) {
            ...
            if (var6 == -1) {
               ...
               this.craftTimer.reset();
               this.setState(NeVasilekFarmilka.State.GRIND_TAKE);
            }
         }
      }
   }
   ...
   private void handleAnvilOpen() {
      if (mc.field_1755 == null && this.actionTimer.finished(this.delayMs())) {
         ...
         this.beginBlockOpen(var1, NeVasilekFarmilka.State.ANVIL_WAIT);
         this.craftTimer.reset();
         this.anvilScreenOpenedAtMs = System.currentTimeMillis();
         this.setState(NeVasilekFarmilka.State.ANVIL_WAIT);
      }
   }
      ...
      if (mc.field_1755 instanceof class_465 var2) {
         if (var2.method_17577() instanceof class_1706 var4) {
            if (!var4.method_34255().method_7960()) {
               ...
               this.craftTimer.reset();
               this.setState(NeVasilekFarmilka.State.ANVIL_PLACE);
            }
      ...
      if (!(mc.field_1755 instanceof class_465 var2 && var2.method_17577() instanceof class_1706 var3)) {
         ...
         if (this.anvilLeftSlot != -1 && this.anvilRightSlot != -1) {
            ...
            if (var6 == -1 || var5 == -1) {
               ...
               this.craftTimer.reset();
               this.setState(NeVasilekFarmilka.State.ANVIL_COMBINE);
            }
         } else {
            this.closeHandledScreenSafely();
            this.setState(NeVasilekFarmilka.State.IDLE);
   ...
   private void handleAnvilCombine() {
      if (!(mc.field_1755 instanceof class_465 var2 && var2.method_17577() instanceof class_1706 var3)) {
         ...
         if (var3.method_7611(2).method_7677().method_7960()) {
            ...
            mc.field_1761.method_2906(var3.field_7763, 2, 0, class_1713.field_7794, mc.field_1724);
            this.craftTimer.reset();
            this.setState(NeVasilekFarmilka.State.ANVIL_TAKE);
         }
      }
   }
   ...
   private void handleSmithOpen() {
      if (mc.field_1755 == null && this.actionTimer.finished(this.delayMs())) {
         ...
         if (var1 == null) {
            ...
            this.beginBlockOpen(var1, NeVasilekFarmilka.State.SMITH_WAIT);
            this.craftTimer.reset();
            this.smithScreenOpenedAtMs = System.currentTimeMillis();
            this.setState(NeVasilekFarmilka.State.SMITH_WAIT);
         }
      }
   }
      ...
      if (mc.field_1755 instanceof class_465 var2) {
         if (var2.method_17577() instanceof class_4862 var4) {
            if (!var4.method_34255().method_7960()) {
               ...
               this.craftTimer.reset();
               this.setState(NeVasilekFarmilka.State.SMITH_PLACE);
            }
         ...
         if (var10 != -1 && var5 != -1) {
            ...
            if (var6 != -1 && var7 != -1) {
               ...
               if (!this.moveStackToSlot(var3, var6, 1)) {
                  ...
                  this.craftTimer.reset();
                  this.setState(NeVasilekFarmilka.State.SMITH_TAKE);
               }
            } else {
               this.closeHandledScreenSafely();
               this.setState(NeVasilekFarmilka.State.IDLE);
         ...
         if (this.sellTimer.finished(200L)) {
            ...
            if (var3 <= 0L) {
               if (this.sellPriceMode.is(this.priceMarket)) {
                  ...
                  MessageUtility.info(class_2561.method_43470("[NeVasilekFarmilka] Invalid fixed price."));
                  this.setState(NeVasilekFarmilka.State.IDLE);
               }
            ...
            if (var5 == -1) {
               MessageUtility.info(class_2561.method_43470("[NeVasilekFarmilka] Hotbar is full."));
               this.setState(NeVasilekFarmilka.State.IDLE);
               return;
            }
            ...
            this.sellTimer.reset();
            this.setState(NeVasilekFarmilka.State.SELL_WAIT);
         }
      } else {
         this.setState(NeVasilekFarmilka.State.IDLE);
      }
      ...
      if (!this.relistPending) {
         ...
         switch (this.state) {
            case RELIST_OPEN_AH:
               ...
               this.sendChat("/ah");
               this.setState(NeVasilekFarmilka.State.RELIST_WAIT_AH);
               this.relistFlowTimer.reset();
               break;
            case RELIST_WAIT_AH:
               if (this.isAuctionMainScreen()) {
                  this.setState(NeVasilekFarmilka.State.RELIST_CLICK_STORAGE);
                  this.relistFlowTimer.reset();
                  return;
               }
            ...
            case RELIST_CLICK_STORAGE:
               ...
               if (var4 instanceof class_1703) {
                  ...
                  if (var5 != -1 && this.clickMenuSlot(var4, var5)) {
                     this.setState(NeVasilekFarmilka.State.RELIST_WAIT_STORAGE);
                     this.relistFlowTimer.reset();
                  }
               }
               break;
            case RELIST_WAIT_STORAGE:
               if (this.isStorageScreen()) {
                  this.setState(NeVasilekFarmilka.State.RELIST_CLICK_RELIST);
                  this.relistFlowTimer.reset();
                  return;
               }
   ...
   private void buildPriceRequests() {
      ...
      this.priceIndex = 0;
      if (this.shouldPriceCheck()) {
         this.priceRequests.add(new NeVasilekFarmilka.PriceRequest("lapis", this.buildSearchName(new class_1799(class_1802.field_8759)), false, false));
         this.priceRequests.add(new NeVasilekFarmilka.PriceRequest("xp", this.getXpSearchQuery(), false, true));
         if (this.isPickaxeMode()) {
            this.priceRequests.add(new NeVasilekFarmilka.PriceRequest("diamond", this.buildSearchName(new class_1799(class_1802.field_8477)), false, false));
            this.priceRequests.add(new NeVasilekFarmilka.PriceRequest("wood", this.getWoodSearchQuery(), false, false));
            ...
            if (var2 != null) {
               this.priceRequests.add(new NeVasilekFarmilka.PriceRequest("base", this.buildSearchName(new class_1799(var2)), false, false));
            }
         }

         class_1792 var1;
         if (this.sellPriceMode.is(this.priceMarket) && (var1 = this.getTargetOutputItem()) != null) {
            this.priceRequests.add(new NeVasilekFarmilka.PriceRequest("output", this.buildSearchName(new class_1799(var1)), true, false));
         }
      }
   }
   ...
   private boolean needsPriceUpdate() {
      if (!this.shouldPriceCheck()) {
         ...
         for (NeVasilekFarmilka.PriceRequest var2 : this.priceRequests) {
            NeVasilekFarmilka.PriceCache var3 = this.priceCache.get(var2.key);
   ...
   private long getUnitPrice(String var1) {
      NeVasilekFarmilka.PriceCache var2 = this.priceCache.get(var1);
      if (var2 != null && var2.isValid()) {
         return var2.price;
      } else {
   ...
   private void buildBuyRequests() {
      if (this.isSwordMode()) {
         ...
         if (var5 > 0) {
            if (this.isPickaxeMode()) {
               if (this.countItem(class_1802.field_8477) < 3 * var5) {
                  this.buyRequests
                     .add(
                        new NeVasilekFarmilka.BuyRequest(
               ...
               if (this.countItem(class_1802.field_8600) < 2 * var5 && var8 > 0) {
                  this.buyRequests.add(new NeVasilekFarmilka.BuyRequest("wood", this.getWoodSearchQuery(), null, var8));
               }
            } else if (this.isSwordMode()) {
               ...
               if (this.countItem(class_1802.field_8600) < 1 * var5 && var14 > 0) {
                  this.buyRequests.add(new NeVasilekFarmilka.BuyRequest("wood", this.getWoodSearchQuery(), null, var14));
               }

               if (this.shouldUpgradeFinalSword() && this.countItem(class_1802.field_22020) <= 0) {
                  this.buyRequests
                     .add(new NeVasilekFarmilka.BuyRequest("netherite", "Незеритовый слиток", class_1802.field_22020, 1 + (int)Math.floor(Math.random() * 4.0)));
               }
            } else {
               class_1792 var10 = this.getTargetBaseItem();
               if (var10 != null && this.countItem(var10) < 1) {
                  this.buyRequests.add(new NeVasilekFarmilka.BuyRequest("base", this.buildSearchName(new class_1799(var10)), var10, 1));
               }
            }
         }
         ...
         if (this.needsXp() && var13 < var11) {
            this.buyRequests.add(new NeVasilekFarmilka.BuyRequest("xp", this.getXpSearchQuery(), class_1802.field_8287, var11, true));
         }
      }
   }
   ...
   private void prepareCraftStage() {
      if (!this.isPickaxeMode() && !this.isSwordMode()) {
         ...
         if (this.isPickaxeMode()) {
            if (var1 < 2) {
               this.craftStage = this.countPlanks() < 2 ? NeVasilekFarmilka.CraftStage.PLANKS : NeVasilekFarmilka.CraftStage.STICKS;
            } else {
               this.setCraftStage(NeVasilekFarmilka.CraftStage.PICKAXE);
            ...
            this.setCraftStage(NeVasilekFarmilka.CraftStage.SWORD);
      ...
      if (!this.isPickaxeMode() || this.countItem(class_1802.field_8377) >= Math.round(this.countFarm.getCurrentValue())) {
         ...
         if (!this.placeOneCraftIngredient(var1, var0 -> var0.method_7909() == class_1802.field_8477, 0)) {
            ...
            return !this.placeOneCraftIngredient(var1, var0 -> var0.method_7909() == class_1802.field_8600, 7)
               ? false
               : this.takeCraftingResult(var1, class_1802.field_8377) || this.isPickaxeRecipePlaced(var1);
         }
      }
   }
   ...
   private int getSwordEnchantLevel(class_1799 var1, NeVasilekFarmilka.SwordEnchantRequirement var2) {
      int var3 = 0;
   ...
   private void stopForFullInventory() {
      MessageUtility.info(class_2561.method_43470("[NeVasilekFarmilka] Inventory full. Stopping."));
   ...
   private void beginBlockOpen(class_2338 var1, NeVasilekFarmilka.State var2) {
      if (var1 != null) {
   ...
   private boolean isRelistState() {
      return this.state == NeVasilekFarmilka.State.RELIST_OPEN_AH
         || this.state == NeVasilekFarmilka.State.RELIST_WAIT_AH
         || this.state == NeVasilekFarmilka.State.RELIST_CLICK_STORAGE
         || this.state == NeVasilekFarmilka.State.RELIST_WAIT_STORAGE
         || this.state == NeVasilekFarmilka.State.RELIST_CLICK_RELIST;
   }
   ...
   private void buildSwordBuyRequests() {
      ...
      if (this.countSharpnessSwords() <= 0) {
         this.buyRequests.add(new NeVasilekFarmilka.BuyRequest("sword_sharp", "незеритовый меч", null, 1, false));
      }

      if (this.countCleanNetheriteSwords() <= 0 && this.countPoisonSwords() <= 0) {
         this.buyRequests.add(new NeVasilekFarmilka.BuyRequest("sword_clean", "незеритовый меч", null, 1, false));
      }

      int var3 = this.requiredXpBottles();
      int var4 = this.countItem(class_1802.field_8287);
      if (this.needsXp() && var4 < var3) {
         this.buyRequests.add(new NeVasilekFarmilka.BuyRequest("xp", this.getXpSearchQuery(), class_1802.field_8287, var3, true));
      }
   }
         ...
         if (this.tryConfirmPurchase()) {
            ...
            if (mc.field_1755 instanceof class_465 var2) {
               ...
               if (!var3.method_34255().method_7960()) {
                  ...
                  NeVasilekFarmilka.BuyRequest var4 = this.buyRequests.get(this.buyIndex);
   ...
   private static final class QueueCoordinatorServer {
      private final int port;
      private final Map<String, NeVasilekFarmilka.QueuePeerSession> peers = new ConcurrentHashMap<>();
      ...
      private void closeAll() {
         ...
         for (NeVasilekFarmilka.QueuePeerSession session : this.peers.values()) {
            session.close();
         }
      ...
      private void startPeerWorker(Socket socket) {
         Thread worker = new Thread(() -> {
            NeVasilekFarmilka.QueuePeerSession session = null;
            ...
            try {
               ...
               if (!id.isBlank()) {
                  session = new NeVasilekFarmilka.QueuePeerSession(socket, reader, writer, id, main, this.joinCounter.getAndIncrement());
                  NeVasilekFarmilka.QueuePeerSession old = this.peers.put(id, session);
                  if (old != null) {
                     old.close();
                  }
      ...
      private void tickQueue() {
         long now = System.currentTimeMillis();
         List<NeVasilekFarmilka.QueuePeerSession> toRemove = new ArrayList<>();

         for (NeVasilekFarmilka.QueuePeerSession peer : this.peers.values()) {
            if (now - peer.lastSeenAtMs > 7000L) {
               toRemove.add(peer);
            }
         }

         for (NeVasilekFarmilka.QueuePeerSession peerx : toRemove) {
         ...
         NeVasilekFarmilka.QueuePeerSession holder = this.currentHolderId.isEmpty() ? null : this.peers.get(this.currentHolderId);
         ...
         if (this.currentHolderId.isEmpty()) {
            List<NeVasilekFarmilka.QueuePeerSession> ready = new ArrayList<>();

            for (NeVasilekFarmilka.QueuePeerSession peerx : this.peers.values()) {
               if (peerx.wantTurn) {
                  ready.add(peerx);
               }
            }

            ready.sort(Comparator.comparingLong(NeVasilekFarmilka.QueuePeerSession::orderKey));
            if (!ready.isEmpty()) {
               NeVasilekFarmilka.QueuePeerSession next = null;

               for (NeVasilekFarmilka.QueuePeerSession peerxx : ready) {
      ...
      private void broadcastStatus() {
         List<NeVasilekFarmilka.QueuePeerSession> orderedReady = new ArrayList<>();
         ...
         orderedReady.sort(Comparator.comparingLong(NeVasilekFarmilka.QueuePeerSession::orderKey));
         if (!this.currentHolderId.isEmpty()) {
            int holderIndex = -1;
            ...
            if (holderIndex > 0) {
               List<NeVasilekFarmilka.QueuePeerSession> rotated = new ArrayList<>(orderedReady.size());
